    ...
    >
      ...
      {numSelected > 0 ? (
        ...
        <Tooltip title="Invite New Volunteer">
          <IconButton aria-label="invite volunteer"></IconButton>
        </Tooltip>
      )}
    </Toolbar>
  );
...
function VolunteersTabPending(props) {
  ...
  const pendingVolunteers = props.store.pendingInvites.map((item, index) => {
    return {
      email: item.email,
    };
  });
  ...
  const [list, setList] = useState(pendingVolunteers);
  ...
  const handleSearchChange = (event) => {
    ...
    setList(
      pendingVolunteers.filter((el) => el.name.includes(event.target.value))
    );
  };
...
export default connect(mapStoreToProps)(VolunteersTabPending);
